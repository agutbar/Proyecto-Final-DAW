...
public class EditarElementos {
	...
	public String editarHijo(Model model, @RequestParam Long id_hijo, @RequestParam String nombre, @RequestParam String apellidos, @RequestParam LocalDate nacimiento) {
		...
		Hijo hijoA = repositorioHijo.buscarPorID(id_hijo);
		Hijo hijo = new Hijo();
	...
	@PostMapping("/datos")
	public String editarUsuario(Model model, @RequestParam String nombre, @RequestParam String apellidos, @RequestParam String telefono, @RequestParam String direccion, @RequestParam String correo, @RequestParam String usuario, @RequestParam String password) {
		Cliente usuarioEditado = (Cliente) model.getAttribute("usuario");
		
		if ((usuarioEditado.getCorreo() != correo && repositorioCliente.findByCorreo(correo).size() != 0) || (usuarioEditado.getUsuario() != usuario && repositorioCliente.findByUsuario(usuario).size() != 0)) {
			...
			model.addAttribute("usuario", usuarioEditado.getUsuario());
			model.addAttribute("confirmacion", "Los datos han sido editados correctamente");
			return "registrado/datos";
	    }
	}
	...
	public String editarTrabajador(Model model, @RequestParam Long id_trabajador, @RequestParam String nombre, @RequestParam String apellidos, @RequestParam LocalDate nacimiento, @RequestParam String telefono, @RequestParam String direccion, @RequestParam String correo, @RequestParam String codigo, @RequestParam String password) {
		Trabajador trabajadorEditado = repositorioTrabajador.buscarPorID(id_trabajador);
		Trabajador trabajadorActivo = (Trabajador) model.getAttribute("trabajador");
		
		if ((trabajadorEditado.getCorreo() != correo && repositorioTrabajador.findByCorreo(correo).size() != 0) || (trabajadorEditado.getCodigo() != codigo && repositorioTrabajador.findByCodigo(codigo).size() != 0)) {
	        model.addAttribute("error", "Algunos de los nuevos datos introducidos ya están en uso por otros usuarios");
			...
			if (trabajadorEditado.getId().equals(trabajadorActivo.getId())) {
				model.addAttribute("trabajador", trabajadorEditado.getCodigo());
			}
			return "administrador/listado";
	    }
	}
