...
public class CRRedireccion {
	...
	private static final String EDITAR_DATOS = "registrado/formulario-edicion/editar-datos";
	private static final String EDITAR_HIJO = "registrado/formulario-edicion/editar-hijo";
	...
	@GetMapping("/inicio") 
	public String mostrarInicioRegistrado(HttpServletRequest request) { 
		...
		List <Revision> listado_citas = new ArrayList<>();
	...
	public String mostrarCitas(HttpSession session) { 
		...
		LocalDate hoy = LocalDate.now();
		List <Revision> citas = usuario.getCitas();
		List <Revision> revisiones_pendientes = new ArrayList<>(), revisiones_completadas = new ArrayList<>();
		for (Revision cita : citas) {
		...
		System.out.println(revisiones_pendientes);
		session.setAttribute("usuario", usuario);
