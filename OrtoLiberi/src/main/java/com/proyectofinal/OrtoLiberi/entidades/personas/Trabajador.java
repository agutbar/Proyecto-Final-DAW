...
public class Trabajador {
	...
	@NotNull(message = "El estado debe ser activo, de baja o inactivo")
	@Pattern(regexp= "^(activo|de-baja|inactivo)$")
    ...
    @Size(min = 3, max = 10, message = "El codigo debe tener entre 3 y 10 caracteres, incluyendo letras y números")
    private String codigo;
    
    @NotBlank(message = "La contraseña es obligatorio")
    //@Size(min = 8, max = 12, message = "La contraseña debe tener entre 8 y 12 caracteres")
    //@Pattern(regexp = "^(?=.*[A-Z])(?=.*[a-z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@_$!%-*?&]+$", message = "La contraseña debe incluir al menos una mayúscula, una minúscula, un número y un carácter especial")
    private String password;
    
    @OneToMany(mappedBy = "vendedor", cascade = CascadeType.ALL, orphanRemoval = true, fetch = FetchType.EAGER)
    private List<Compra> ventas = new ArrayList<>();
    
    @OneToMany(mappedBy = "responsable", cascade = CascadeType.ALL, orphanRemoval = true, fetch = FetchType.EAGER)
    private List<Fabricacion> trabajos = new ArrayList<>();
    
    @OneToMany(mappedBy = "tecnico", cascade = CascadeType.ALL, orphanRemoval = true, fetch = FetchType.EAGER)
	...
	public Trabajador(Long id,
			...
			@NotNull(message = "El estado debe ser activo, de baja o inactivo") @Pattern(regexp = "^(activo|de-baja|inactivo)$") String estado,
			...
			@NotBlank(message = "El codigo es obligatorio") @Size(min = 3, max = 10, message = "El codigo debe tener entre 3 y 10 caracteres, incluyendo letras y números") String codigo,
	...
	public String toString() {
		return "Trabajador [id=" + id + ", nombre=" + nombre + ", apellidos=" + apellidos + ", nacimiento=" + nacimiento
				+ ", telefono=" + telefono + ", correo=" + correo + ", direccion=" + direccion + ", codigo=" + codigo
				+ ", password=" + password + "]";
	}
