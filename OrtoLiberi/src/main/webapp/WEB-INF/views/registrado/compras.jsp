										...
										<table class="table table-striped">
											...
											<tbody>
												<tr th:each="compra : ${cliente.compras}">
													...
													<td th:text="${compra.pago}"></td>
													<td th:text="${compra.tipo}"></td>
													<td th:text="${compra.vendedor}"></td>
													<td th:text="${fn:length(compra.productos)}"></td>
													<td>
														<a href="/compra/ver-productos{id}(id=${compra.id})" class="btn btn-info btn-sm">Ver Productos</a>
													</td>
												</tr>
											</tbody>
										</table>
										...
										<table class="table table-striped">
											...
											<tbody>
												<tr th:each="pedido : ${cliente.pedidos}">
													<td th:text="${pedido.peticion}"></td>
													<td th:text="${pedido.prevision}"></td>
													<td th:text="${pedido.estado}"></td>
													<td th:text="${pedido.precio}"></td>
													<td th:text="${fn:length(pedido.productos)}"></td>
													<td>
														<a href="/pedido/ver-detalles/{id}(id=${pedido.id})}" class="btn btn-info btn-sm">Ver Detalles</a>
														<c:if test="${pedido.estado == 'En cola' || pedido.estado == 'En tramite'}">
															<a href="/pedido/editar/{id}(id=${pedido.id})}" class="btn btn-warning btn-sm">Editar</a>
															<a href="/pedido/cancelar/{id}(id=${pedido.id})}" class="btn btn-danger btn-sm">Cancelar Pedido</a>
														</c:if>
														<c:if test="${pedido.estado == 'Enviado'}">
															<a href="/pedido/completar/{id}(id=${pedido.id})}" class="btn btn-danger btn-sm">Confirmar Recepción</a>
														</c:if>
													</td>
												</tr>
											</tbody>
