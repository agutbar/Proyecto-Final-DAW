                ...
                <div class="col-sm-8 offst-sm-2">
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        <c:forEach var="hijo" items="${cliente.menores}">
							<li class="nav-item">
								<a class="nav-link" id="'children -' + ${hijo.id}" data-bs-toggle="tab" href="#${hijo.id}" role="tab" aria-controls="${hijo.id}" aria-selected="false">${hijo.nombre}</a>
							</li>
						</c:forEach>
                    </ul>
					<div class="tab-content" id="myTabContent">
						<c:forEach var="hijo" items="${cliente.menores}">
	                        <div class="tab-pane fade" id="${hijo.id}" role="tabpanel" aria-labelledby="'children -' + ${hijo.id}">
	                            <dl class="row">
	                                <dt class="col-sm-3">Nombre Completo</dt>
	                            ...
	                            <p>
	                                <a id="ver-ficha-hijo" href="/ver-hijo/${hijo.id}" class="btn btn-info">Acceder a Su Ficha Completa</a>
	                            </p>
	                        </div>
						</c:forEach>
                    </div>
