...
<body>
    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item active" aria-current="page">Editar Producto de Fabricacion</li>
            </ol>
        </nav>
    </div>
    ...
    <section class="wrap" id="new-user">
        <div class="container">
            <h1>Editar Producto de Fabricacion</h1>
            <p class="lead">Edita la ficha de un producto que estás fabricando en taller</p>
            <div class="card">
                <div class="card-body">
                    <form id="formulario_editar_fabricacion" method="post" action="/editar/fabricacion">
						<div class="form-group">
							<label for="inputId" hidden>ID Fabricacion</label>
							<input type="text" class="form-control" id="inputId" name="id" value="${fabricacion.id}" hidden required/>
						</div>
						<div class="form-group">
							<label for="inputMaterial">Material</label>
							<select class="form-control" id="material" th:field="*{material.id}" required>
								<option value="">Seleccione un material</option>
								<c:forEach var="material" items="${materiales}">
									<option value="${material.id}">${material.nombre}</option>
								</c:forEach>
							</select>
							<span class="error-formulario" id="errorMaterial" style="font-style: italic; color: red;"></span>
						</div>
						<div class="form-group">
							<label for="inputCantidad">Cantidad</label>
						    <input type="number" class="form-control" id="inputCantidad" name="cantidad" step="1" min="1" required/>
							<span class="error-formulario" id="errorCantidad" style="font-style: italic; color: red;"></span>
						</div>
                        <button type="submit" class="btn btn-success btn-block" id="editar_fabricacion">Añadir Material</button>
                    </form>
			...
			<table class="table table-striped">
				...
				<tbody>
					<c:forEach var="material" items="${fabricacion.materiales}">
						<tr>
							<td>${material.key.nombre}</td>
        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form>
                        <button type="submit" class="btn btn-success btn-block"><a href="/administrador/editar-fabricacion">Volver</a></button>
                    </form>
                </div>
            </div>
        </div>
