	        ...
	        <div class="card">
	        	<div class="card-body">
	            	<form id="formulario_pedido_proveedor" method="post" action="/nuevo/pedidoProveedor">
							    ...
							    <tbody>
							    	<c:forEach var="producto" items="${pedido.productos}">
							        	<tr>
											...
											<td><a th:href="@{/producto/editar/{id}(id=${producto.id})}" class="btn btn-warning btn-sm">Editar</a></td>
											<td><a th:href="@{/producto/eliminar/{id}(id=${producto.id})}" class="btn btn-danger btn-sm">Eliminar</a></td>
							         	</tr>
									</c:forEach>
								</tbody>
							</table>
						...
						<div class="form-group">
							<label for="inputMateriales">Listado de Materiales</label>
							<button type="button" class="btn btn-success btn-block" id="sumar_material">Añadir Nuevo Material</button>
							<table class="table table-striped">
								<thead>
									<tr>
								...
								<tbody>
									<c:forEach var="material" items="${pedido.materiales}">
										<tr>
											<td>${material.nombre}</td>
											<td>${material.precio}</td>
											<td>${material.cantidad}</td>
											<td>${material.precio * material.cantidad}</td>
											<td><a th:href="@{/material/editar/{id}(id=${material.id})}" class="btn btn-warning btn-sm">Editar</a></td>
											<td><a th:href="@{/material/eliminar/{id}(id=${material.id})}" class="btn btn-danger btn-sm">Eliminar</a></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
						<span class="error-formulario" id="errorMateriales" style="font-style: italic; color: red;"></span>
						</div>
						<div class="form-group">
	                    ...
	                    <button type="button" class="btn btn-success btn-block" id="registrar_pedido">Hacer Pedido</button>
	           		</form>
